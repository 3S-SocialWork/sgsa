...
var __spreadArrays =
	...
	function() {
		...
		for (var r = Array(s), k = 0, x = 0; i < il; x++)
			for (var a = arguments[x], j = 0, jl = a.length; j < jl; j++, k++)
				r[k] = a[j];
		return r;
	};
...
const msgTransform = (message, level) => {
	let msg = [];
	...
	for (let field in message) {
		if (
			typeof message[field] === "object" &&
			typeof message[field][0] === "object"
		) {
			msg.push(field);
			msg.push(msgTransform(message[field][0], true));
		}
		if (
			typeof message[field] === "object" &&
			typeof message[field][0] !== "object"
		) {
			msg.push((level ? "  " : "") + field + ": " + message[field]);
		}
	}

	return String(msg.join("\r\n"));
};
		...
		.then(function(_a) {
			...
			if (status < 200 || status >= 300) {
				return Promise.reject(
					new HttpError(
						(json && msgTransform(json.message)) || statusText,
						status,
						json,
					),
				);
